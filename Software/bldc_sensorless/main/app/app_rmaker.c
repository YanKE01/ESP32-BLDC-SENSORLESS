 ...
 * @return {*}
 */
static esp_err_t app_rmaker_write_cb(const esp_rmaker_device_t *device, const esp_rmaker_param_t *param, const esp_rmaker_param_val_t val, void *priv_data, esp_rmaker_write_ctx_t *ctx)
{
    if (ctx)
    {
...
void app_rmaker_init()
{
    ...
    // 添加控件-滑动条
    esp_rmaker_param_t *speed_param = esp_rmaker_param_create(ESP_RMAKER_DEF_SPEED_NAME, ESP_RMAKER_PARAM_SPEED,
                                                              esp_rmaker_int(400), PROP_FLAG_READ | PROP_FLAG_WRITE);
    if (speed_param)
    {
        esp_rmaker_param_add_ui_type(speed_param, ESP_RMAKER_UI_SLIDER);
        esp_rmaker_param_add_bounds(speed_param, esp_rmaker_int(400), esp_rmaker_int(800), esp_rmaker_int(1));
    }
    esp_rmaker_device_add_param(fan_device, speed_param);
    esp_rmaker_device_assign_primary_param(fan_device, power_param); // 首页控件
